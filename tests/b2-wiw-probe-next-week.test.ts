...
<div id="dlg" role="dialog" aria-label="Export Schedule" hidden>
  <button aria-label="close" onclick="document.getElementById('dlg').hidden=true; closePicker()">&times;</button>
  <button id="s" aria-label="Start Date" onclick="openPicker('s')"></button>
  <button id="e" aria-label="End Date" onclick="openPicker('e')"></button>
...
function set(value) {
  ...
  fetch('/pick');
  if (STICKY) localStorage.setItem('range', JSON.stringify([document.getElementById('s').textContent, document.getElementById('e').textContent]));
  closePicker();
}
function openPicker(id) {
  ...
  if (KIND === 'typed') {
    ...
    i.addEventListener('keydown', (ev) => { if (ev.key === 'Enter') set(i.value); });
...
function render() {
  ...
  const next = NO_NEXT ? '' : TRAP ? '<button onclick="fetch(\\'/clear\\'); month.setMonth(month.getMonth()+1); render()">Clear and next</button>'
    : '<button aria-label="Next Month" onclick="month.setMonth(month.getMonth()+1); render()">&gt;</button>';
...
describe("B2 probe-next-week (fake scheduler)", { timeout: 60_000 }, () => {
  ...
  it("sticky dialog: the reopen check reports the next week it would hand the timer", async () => {
